...
#[cfg(all(feature = "c", feature = "complete-system-rt"))]
build_error!("The 'c' and 'complete-system-rt' features are not compatible");
...
fn main() {
    ...
    // functions
    //
    // We also need a C compiler if we are building extra intrinsics for testing.
    if (!cfg!(feature = "mangled-names") && cfg!(feature = "c"))
        || cfg!(feature = "complete-system-rt")
    {
        #[cfg(any(feature = "c", feature = "complete-system-rt"))]
...
mod c {
    ...
    pub fn compile(llvm_target: &[&str], target_str: &str) {
        // Don't use a C compiler for these targets:
        //
        // * nvptx - everything is bitcode, not compatible with mixed C/Rust
        if target_str.contains("nvptx") {
            println!("cargo:warning=nvptx target; skipping C build");
            return;
        }
        ...
        let sys_missing_only = cfg!(feature = "complete-system-rt");
        let mut consider_float_intrinsics = true;

        // These platforms have all intrinsics we want to test against, so we don't
        // need to build anything else.
        if sys_missing_only && [""].contains(&target.arch.as_str()) {
            return;
        }

        let mut cfg = create_c_build_config(
            &target,
            target_str,
            sys_missing_only,
        ...
        if sys_missing_only {
            extend_sources_sys_missing_only(&mut sources, &target, consider_float_intrinsics);
        } else {
            extend_sources_full(
                &mut sources,
                llvm_target,
        ...
        let root = match env::var_os("RUST_COMPILER_RT_ROOT") {
            ...
            None => {
                println!("cargo:warning=Defaulting to RUST_COMPILER_RT_ROOT at ./compiler-rt");
                PathBuf::from("./compiler-rt")
            }
        };
        if !root.exists() {
        ...
        if target.arch == "aarch64" && target.env != "msvc" {
            ...
            build_aarch64_out_of_line_atomics_libraries(&src_dir, &mut cfg);
    ...
    /// Set up configuration for the C libraries
    fn create_c_build_config(
        target: &Target,
        target_str: &str,
        sys_missing_only: bool,
        consider_float_intrinsics: &mut bool,
    ) -> cc::Build {
        ...
        if target.arch == "aarch64" {
            let cflags_key = format!("CFLAGS_{}", target_str.replace("-", "_"));
        ...
        if target.vendor == "apple" && sys_missing_only {
            cfg.define("CRT_HAS_TF_MODE", "1");
            cfg.define("CRT_HAS_IEEE_TF", "1");
            cfg.define("CRT_HAS_F128", "1");
            cfg.define("CRT_HAS_128BIT", "1");
            // A hacky way to get something with the correct f128 ABI (uint128_t size and
            // alignment but passed with the float CC) on platforms where it isn't defined.
            cfg.define("tf_float", "double __attribute__((vector_size(16)))");
        }

        cfg
    }

    /// Add functions that are implemented in Rust's `compiler-rt` and exist in LLVM `compiiler-rt`
    /// sources, but are not built/available on the host system.
    ///
    /// This is only needed for testing.
    fn extend_sources_sys_missing_only(
        sources: &mut Sources,
        target: &Target,
        consider_float_intrinsics: bool,
    ) {
        if target.vendor == "apple" && consider_float_intrinsics {
            sources.extend(&[
                ("__addtf3", "addtf3.c"),
                ("__multf3", "multf3.c"),
                ("__subtf3", "subtf3.c"),
    ...
    /// Add functions that are missing from Rust's `compiler-rt`, or that we would like to
    /// replace with a more performant version.
    fn extend_sources_full(
        sources: &mut Sources,
        llvm_target: &[&str],
        target: &Target,
        consider_float_intrinsics: bool,
        ...
        if target.os != "ios"
            && target.os != "watchos"
            && target.os != "tvos"
            && (target.vendor != "apple" || target.arch != "x86")
        {
            sources.extend(&[
                ("__absvti2", "absvti2.c"),
                ("__addvti3", "addvti3.c"),
